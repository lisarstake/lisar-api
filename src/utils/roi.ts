import { getAverageBlockTime, DEFAULT_AVERAGE_L1_BLOCK_TIME } from './blockTime';
...
export type ROIParams = {
  ...
  rewardParams: {
    ...
    roundLength: number;
    // optional override for average L1 block time (seconds)
    averageL1BlockTime?: number;
...
export async function calculateROI({
  ...
  // determine average L1 block time (seconds) — use provided override else fetch
  const resolvedAverageL1BlockTime =
    typeof averageL1BlockTime === 'number'
      ? averageL1BlockTime
      : await getAverageBlockTime();

  const averageSecondsPerRound = roundLength * (resolvedAverageL1BlockTime || DEFAULT_AVERAGE_L1_BLOCK_TIME);
